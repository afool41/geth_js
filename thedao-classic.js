...
function thedao_listProposals(startProposal, endProposal) {
  ...
  for (i = startProposal; i <= endProposal; i++) {
    ...
    if ((votingDeadline*1000) < timeNow) {
        passDue = "voting ENDs!";
        if (newCurator)
           newDAOAddress = " (new DAO : " + thedao.getNewDAOAddress(i) + ")";
    } else {
  ...
  return("Total DAO tokens : " + totalSupply);
}
