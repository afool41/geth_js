...
function tempdao_listProposals(startProposal, endProposal) {
  var numberOfProposals = tempdao.numberOfProposals();
  ...
  var totalSupply = tempdao.totalSupply().dividedBy(1e16);

  for (i = startProposal; i <= endProposal; i++) {
    var proposal = tempdao.proposals(i);  
    ...
    if ((votingDeadline*1000) < timeNow) {
        passDue = "voting ENDs!";
        if (newCurator)
           newDAOAddress = " (new DAO : " + tempdao.getNewDAOAddress(i) + ")";
    } else {
  ...
  return("Total TempDAO tokens : " + totalSupply);
}

function tempdao_status() {
  var myEth = 6772.203185900849465034;
  var creationEnds = tempdao.closingTime();

  var timeNow = Date.now();
  console.log("Temp DAO addr : " + tempdao_addr);
  ...
  var bal = web3.fromWei(eth.getBalance(tempdao_addr), "ether");
